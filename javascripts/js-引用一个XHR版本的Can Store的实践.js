...
/*
//XMLHttpRequest版本
let urlProduct = 'https://github.com/mdn/learning-area/blob/main/javascript/apis/fetching-data/can-store/products.json'

let request = new XMLHttpRequest();
request.open('GET',urlProduct);
request.responseType = 'json';

request.onload = function (){
  if(request.status === 200){//或者request.readyState ===4
  let canProduct = request.response;
  initialize(canProduct);
 }else{
  throw new Error(`Http error :${request.status}`);
//学习社区参考答案  console.log('Network request for product,json is not response' + request.status +':' + request.statusText); 
 }
}
request.send();
*/
...
function initialize(products) {
  ...
  // a search running to select the category(类别的意思) of products we want to display
  ...
  function selectCategory(e) {
    ...
    if (category.value === lastCategory && searchTerm.value.trim() === lastSearch) {
      ...
      // selectProducts()
        ...
        // Filter(筛选的意思) categoryGroup to contain only products whose type includes the category
  ...
  function fetchBlob(product) {
    ...
    const url = `images/${product.image}`;//不是本地服务器引用不了图片,关键图片信息引用代码块
    //这个获取图片的代码的路径其实就是文件夹加上文件名，只不过这个图像文件名以JSON的形式来写代码获取,product就是一个JSON文件里的image文件路径名
...
/*
      //XMLHttpRequest版本
      let secondUrl = `images/${product.image}`;

      let request = new XMLHttpRequest();
      request.open('GET',secondUrl);
      request.responseType = 'blob';  //存储图像文件的路径名

    request.onload = function (){       
      if(request.status === 200){
        let blob = request.response; 
        let objectURL = URL.createObjectURL(blob);//根据存储的图像文件路径名进行图像照片的获取
        showProduct(blob,product);
      }else{
    //学习社区参考版本  
     console.log('Network request for product.blob is not response' + request.status + ':' + request.statusText);
     
     console.error(`GET peoblem:${err.message}`);
    //另外一种响应错误代码 throw new Error(`Http error: ${request.status}`);
   }
  };
request.send();
*/
